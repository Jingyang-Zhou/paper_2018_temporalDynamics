...
dataLoc = fullfile(temporalRootPath, 'data', 'ECoG', 'Figure1_ACE_Data.xlsx');
a = xlsread(dataLoc);
...
pred = [];
for k = 1 : 3
    prms(k, :) = fminsearch(@(x) dn_fitModel_contrast(x, cellRsp{k}', time{k}, contrast_levels, stim), init);
...
figure (2), clf
for k = 1 : 3
    subplot_tight(2, 3, k, 0.07)
    set(gca, 'ColorOrder',copper(9)); hold on
    plot(time{k}, cellRsp{k}(:, 2 : end), '-', 'linewidth', 2), axis tight, axis square, box off, 
    set(gca, 'ytick', [0, 1])
   
    subplot_tight(2, 3, 3 + k, 0.07)
    set(gca, 'ColorOrder', copper(9)); hold on
    plot(time{k}, pred{k}(2 : end, :), '-'), axis tight, axis square, box off, set(gca, 'ytick', [0, 1])
end
