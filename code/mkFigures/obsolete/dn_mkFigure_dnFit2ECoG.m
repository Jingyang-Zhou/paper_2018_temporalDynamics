...
saveFigures = 0; % 0 if do not want to save figure
...
subplot(1, 4, 1), ylabel('fractional signal change'), xlabel('time (s)')
...
subplot(1, 4, 1), legend('data', 'prd. with shift', 'prd without shift'),

% COMPARE RESPONSE SHAPES ACROSS ROIS -------------------------------------
fg2 = figure (2); clf, colorMatrix = copper(nRois);
for k = 1 : 2
    subplot(1, 2, k),
...
subplot(1, 2, 1), ylim([100, 160]), subplot(1, 2, 2), ylim([0, 0.14])
...
for k = 1 : 4
    ...
    for iroi = 1 : nRois
        plot([iroi, iroi], s_prm(iroi, :, k), 'k-')
    end
    xlim([0.5, nRois + 0.5])
end
subplot(1, 4, 1), ylim([0, 0.45])
subplot(1, 4, 2), ylim([0, 0.2])
subplot(1, 4, 3), ylim([0, 5])
