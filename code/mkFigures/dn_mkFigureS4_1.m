...
% figure (100), clf
% for k = 1 : 12
%     subplot(4, 3, k), plot(sua.t, sua.dt(:, k))
% end

nCells = [1, 1, 1, 1, 1, 2, 2, 2, 2, 4, 4, 3];

ext_dt(:, 1) = sua.dt(:, 1);

for k = 2 : length(nCells)
   ext_dt = [ext_dt, repmat(sua.dt(:, k), [1, nCells(k)])];
end

ext_nCells = size(ext_dt, 2);
    
%% ANALYZE SUA:

for iBoot = 1 : nBoots
    idx = randi(ext_nCells, [1, nBoots]);
    sua.bsdt(iBoot, :) = normMax(mean(ext_dt(:, idx), 2));
end
...
shadedErrorBar(sua.t, sua.mbsdt, sua.sbsdt, 'b'), xlim([-0.05, 0.25]), ylim([-0.1, 1]), set(gca, 'fontsize', 14)
...
text(0.1, 0.1, sprintf('r^2 = %.3f', sua.r2))
...
figure (1), 
subplot(1, 4, 2), cla, title('MUA (human V2/V3)')
% PLOT THE STIMULUS -------------------------------------------------------
...
text(0.4, 0.1, sprintf('r^2 = %0.3f', mua.r2))
...
figure (10), clf
%patch([85, 175, 175, 85], [100. 100, 10^4, 10^4], 0.9 * ones(1, 3)), hold on
loglog(f1, mean(fft_dt), 'k-', 'linewidth', 3), hold on

% PLOT THE RANGE OF BROADBAND EXTRACTION

loglog(f2, mean(base_dt), '-', 'color', 0.7 * ones(1, 3), 'linewidth', 3), xlim([30, 200]), box off
legend('baseline', 'stimulus dependent'), set(gca, 'fontsize', 16), ylim([100, 10^3.5])
set(gca, 'xtick', [60, 100, 200])

%% EXTRACT BROADBAND

%bands = [75 95; 105 125; 125 145; 155 175]; % line noise is at 50 Hz
bands = [85 95; 105 115; 115 125; 125 135; 135 145; 155 165; 165 175];
...
for iTrial = 1 : nTrials
    for iband = 1 : size(bands, 1)
        ...
        bb0(iTrial, iband, :) = sum(whiten_hilbert(bp_trial).^2, 2);
    end
end
...
plot(t(1 : end-45), lfp.mbb(1 : end-45), 'b-', 'linewidth', 3), axis square
set(gca, 'xaxislocation', 'origin', 'fontsize', 14, 'ytick', [0, 1], 'xtick', [0.15, 0.65], 'xticklabel', [0, 0.5]),
axis tight, ylim([-0.1, 1])

%% FIT THE DN MODEL TO THE LFP BROADBAND

lfp_idx = 1 : length(t) - 45;
...
text(0.4, 0.1, sprintf('r^2 = %.3f', lfp.r2))

%% PLOT ECOG DATA AND MODEL FIT

fg1 = figure (1); subplot(1, 4, 4), cla, title('ECOG broadband (human V1)')
...
% compute r2 between data and prediction
mpred = mean(ecog.prd, 2);
mdata = mean(ecog.dt, 2);
ecog.r2 = corr(mpred, mdata).^2;

text(0.3, 0.1, sprintf('r^2 = %.3f', ecog.r2))


fg1.Position = [250, 1000, 1000, 250];

%% USE ECoG PARAMETERS TO DESCRIBE SUA DATA

ecogPrm = mean(d.prm.ecog.dn.param(1 : 100, :));

% make prediction to SUA stim:
ecog2sua_prd = dn_DNmodel([ecogPrm(1), 0, ecogPrm(2 : 4),0, 1], sua.stim, sua.t);
ecog2sua_prd = ecog2sua_prd ./max(ecog2sua_prd);

figure (1), subplot(1, 4, 1), 
plot(sua.t, ecog2sua_prd, 'k-', 'linewidth', 3)
